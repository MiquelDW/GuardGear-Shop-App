...
declare global {
  // declare global variable 'cachedPrisma' to cache the Prisma client instance across multiple invocations to avoid creating multiple instances in development mode (because of Hot Reload in Next.js), which can improve performance and prevent issues in serverless environments
  var cachedPrisma: PrismaClient;
}

let prisma: PrismaClient;
if (process.env.NODE_ENV === "production") {
  // instantiate DB client to access and read and write data in your DB (happens only once in production environment)
